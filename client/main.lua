...
RegisterNetEvent('esx_spectate:checkC')
AddEventHandler('esx_spectate:checkC', function(names, ids)
    idPlayers = ids
	namePlayers = names
    table.insert(data, {id = idPlayers, name = namePlayers})
end)

RegisterNetEvent('esx_spectate:checkCpos')
AddEventHandler('esx_spectate:checkCpos', function(pos)
	Thepos = pos
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, 163) then
			if group ~= "user" then
				TriggerServerEvent('esx_spectate:checkS')
				Wait(1000)
...
RegisterNUICallback('select', function(data, cb)
	TriggerServerEvent('esx_spectate:checkSpos', data.id)
	Citizen.Wait(1000)
	spectate(data.id)
	SetNuiFocus(false)
	data = {}
...
Citizen.CreateThread(function()
	...
	while true do
	  ...
	  if InSpectatorMode then
		  TriggerServerEvent('esx_spectate:checkSpos', TargetSpectate)
	  end
  end
end)
